...
export function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await signup({ userNick: userNick.trim(), userPhone: userPhone.trim(), userPassword });
      history.push("/");
    } catch (err: any) {
      setError(err?.response?.data?.message || err?.response?.data?.error || Messages.errorUserExists);
    } finally {
      setLoading(false);
    }
  };
  ...
  const inputSx = {
    "& .MuiOutlinedInput-root": {
      borderRadius: "12px", bgcolor: "#FAFAF8", fontSize: "0.95rem", transition: "all 0.2s ease",
      "& fieldset": { borderColor: "rgba(0,0,0,0.1)" },
      "&:hover fieldset": { borderColor: "#C8960C" },
      "&.Mui-focused": { bgcolor: "#FFFFFF", "& fieldset": { borderColor: "#C8960C", borderWidth: "2px" } },
    },
    "& .MuiInputBase-input": {
      py: 1.6, color: "#1C1917",
      "&::placeholder": { color: "#C4BAB4", opacity: 1 },
    },
    "& input:-webkit-autofill": { WebkitBoxShadow: "0 0 0 100px #FAFAF8 inset", WebkitTextFillColor: "#1C1917" },
  };
  ...
  const confirmInputSx = {
    ...
    "& .MuiOutlinedInput-root": {
      ...
      ...(pwMatch    && { "& fieldset": { borderColor: "#22C55E", borderWidth: "2px" } }),
      ...(pwMismatch && { "& fieldset": { borderColor: "#EF4444", borderWidth: "2px" } }),
    },
  };

  const labelSx = {
    fontSize: "0.72rem", fontWeight: 700, color: "#78716C",
    mb: 0.8, letterSpacing: "0.08em", textTransform: "uppercase" as const,
  };

  return (
    <Box sx={{
      minHeight: "100vh", bgcolor: "#FDF8F0",
      ...
      <Box sx={{ width: "100%", maxWidth: 420, position: "relative" }}>
        {/* Card */}
        <Box sx={{
          bgcolor: "#FFFFFF", borderRadius: "24px",
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <Box sx={{ mb: 2.2 }}>
              ...
              <TextField fullWidth value={userNick} onChange={(e) => setUserNick(e.target.value)}
                ...
                InputProps={{ startAdornment: <InputAdornment position="start"><PersonIcon sx={{ color: "#C4BAB4", fontSize: 19 }} /></InputAdornment> }}
                sx={inputSx} />
            </Box>
            ...
            <Box sx={{ mb: 2.2 }}>
              ...
              <TextField fullWidth value={userPhone} onChange={(e) => setUserPhone(e.target.value)}
                placeholder="+998 90 123 45 67" required autoComplete="tel"
                inputProps={{ inputMode: "tel" }}
                InputProps={{ startAdornment: <InputAdornment position="start"><PhoneIcon sx={{ color: "#C4BAB4", fontSize: 19 }} /></InputAdornment> }}
                sx={inputSx} />
            </Box>
            ...
            <Box sx={{ mb: 2.2 }}>
              ...
              <TextField fullWidth type={showPw ? "text" : "password"}
                value={userPassword} onChange={(e) => setUserPassword(e.target.value)}
                placeholder="••••••••" required autoComplete="new-password"
                InputProps={{
                  startAdornment: <InputAdornment position="start"><LockIcon sx={{ color: "#C4BAB4", fontSize: 19 }} /></InputAdornment>,
                  endAdornment: (
            ...
            <Box sx={{ mb: 3.5 }}>
              ...
              <TextField fullWidth type={showCpw ? "text" : "password"}
            ...
            <Button type="submit" fullWidth variant="contained" disabled={loading}
              ...
              {loading ? <CircularProgress size={20} sx={{ color: "#B8AFA8" }} /> : "Create Account"}
            </Button>
            ...
            <Button fullWidth variant="outlined" onClick={() => history.push("/login")} sx={{
              ...
              <Box component="span" sx={{ color: "#C8960C", fontWeight: 700, ml: 0.5 }}>Sign In</Box>
            </Button>
          </form>
        </Box>
